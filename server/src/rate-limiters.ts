...
// Global rate limiter (environment-aware) with GYG supplier API bypass
export const globalLimiter = rateLimit({
  ...
  skip: (req) => {
    // Skip rate limiting for GYG supplier API endpoints (server-to-server communication)
    return req.path.startsWith('/gyg');
  },
  handler: (req, res) => {
