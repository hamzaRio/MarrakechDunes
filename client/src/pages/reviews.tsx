...
import { useState } from "react";
import { useQuery } from "@tanstack/react-query";
import { ReviewType, ActivityType } from "@shared/schema";
...
export default function Reviews() {
  ...
  const { data: reviews = [] } = useQuery<ReviewType[]>({
    queryKey: ["/api/reviews", selectedActivityId],
    queryFn: async () => {
      const url = selectedActivityId 
        ? `/api/reviews?activityId=${selectedActivityId}`
        : "/api/reviews";
      ...
      <section className="bg-moroccan-blue text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-blue-100">
            Discover what our guests say about their Moroccan adventures
          </p>
        </div>
      </section>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    <CardTitle className="text-2xl font-playfair text-moroccan-blue">
                      {selectedActivity ? selectedActivity.name : "All Activities"}
                    </CardTitle>
