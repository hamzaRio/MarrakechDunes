...
# Tests local and remote authentication endpoints
...
Write-Host "`n🔐 MarrakechDunes Authentication Tests" -ForegroundColor Cyan
...
Write-Host "`n2. Auth User (Unauthenticated)" -ForegroundColor Yellow
try {
...
Write-Host "`n3. Login with Ahmed (Admin)" -ForegroundColor Yellow
...
Write-Host "`n4. Auth User (Authenticated as Ahmed)" -ForegroundColor Yellow
try {
...
Write-Host "`n6. Auth User (After Logout)" -ForegroundColor Yellow
try {
    $response = Invoke-WebRequest -Uri "$BaseUrl/api/auth/user" -WebSession $session -UseBasicParsing -TimeoutSec 10
...
Write-Host "`n7. Login with Nadia (Superadmin)" -ForegroundColor Yellow
...
Write-Host "`n8. Auth User (Authenticated as Nadia)" -ForegroundColor Yellow
try {
...
Write-Host "`n9. Login with Yahia (Admin)" -ForegroundColor Yellow
